...
func TestReceiverReconnectsAndConsumesFeed(t *testing.T) {
	...
	updates := make(chan liveTimingUpdate, 1)
	receiver.consume = func(_ context.Context, batch []liveTimingUpdate) error {
	...
	case update := <-updates:
		...
		if update.timestamp != "2026-08-21T10:30:00.034Z" {
			t.Errorf("update timestamp = %q", update.timestamp)
		}
